...
import rclpy
import numpy as np
import pyfri as fri # type: ignore
...
class FRIController(fri.LBRClient):
    def __init__(self, robot_name:str="LBRiiwa7R800"):
        super().__init__()

        rclpy.init(args=None)
        self.__node = rclpy.create_node(f"fri_controller_node")
    ...
    def waitForCommand(self):
        if not self.__position.tolist():
            self.__position = self.robotState().getIpoJointPosition()
    ...
    def command(self):
        ...
        current_pos = self.robotState().getCommandedJointPosition()
